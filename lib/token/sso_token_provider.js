...
AWS.SSOTokenProvider = AWS.util.inherit(AWS.Token, {
  ...
  load: function load(callback) {
    ...
    if (Object.keys(profile).length === 0) {
      throw AWS.util.error(
        new Error('Profile "' + profile + '" not found'),
        { code: 'SSOTokenProviderFailure' }
      );
    } else if (!profile['sso_start_url']) {
      callback(AWS.util.error(
        new Error('Profile "' + profileName + '" is missing required property "sso_start_url".'),
        { code: 'SSOTokenProviderFailure' }
      ));
      return;
    } else if (!profile['sso_region']) {
      throw AWS.util.error(
        new Error('Profile "' + profileName + '" is missing required property "sso_region".'),
        { code: 'SSOTokenProviderFailure' }
      );
    }
    ...
    if (!self.service || self.service.config.region !== profile.sso_region) {
      self.service = new AWS.SSOOIDC({ region: profile.sso_region });
    }
