  ...
  handleRequest: function handleRequest(httpRequest, httpOptions, callback, errCallback) {
    ...
    if (expect === '100-continue') {
      stream.on('continue', function() {
        self.writeBody(stream, httpRequest, httpOptions);
      });
    } else {
      this.writeBody(stream, httpRequest, httpOptions);
    }

    return stream;
  },

  writeBody: function writeBody(stream, httpRequest, httpOptions) {
    var body = httpRequest.body;

    // progress support
    if (body && !httpOptions.disableProgressEvents && WritableStream && ReadableStream) {
