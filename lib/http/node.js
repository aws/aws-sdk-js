...
require('../http');
var OldStreamValidators = require('./response-validator/attachValidators');
    ...
    var stream = http.request(options, function (httpResp) {
      ...
      if (headers && headers['content-length'] && httpRequest.method !== 'HEAD') {
        if (AWS.HttpClient.streamsApiVersion === 2) {
          var ValidationStreams = require('./response-validator/validationStreams');//lazy loading
          var contentLengthCheckerStream = new ValidationStreams.ContentLengthCheckerStream(
          ...
          OldStreamValidators.attachLengthChecker(responseStream, parseInt(headers['content-length'], 10));
        }
      }
      ...
      if (headers && headers['x-amz-transfer-encoding'] && headers['content-length']) {
        ...
        if (AWS.HttpClient.streamsApiVersion === 2) {
          var ValidationStreams = require('./response-validator/validationStreams');
          var integrityCheckerStream = new ValidationStreams.IntegrityCheckerStream(transferEncoding, contentLength);
          responseStream.on('error', function(err) {
          ...
          //TODO
          //may not support integrity check for node version < 0.9.4 / streamAPI version 1
        }
        httpResp.headers['content-length'] -= 16;
        if (httpResp.headers['x-amz-content-range']) {
          httpResp.headers['content-range'] = httpResp.headers['x-amz-content-range'];
        }
      }
