...
AWS.NodeHttpClient = AWS.util.inherit({
  handleRequest: function handleRequest(httpRequest, httpOptions, callback, errCallback) {
    ...
    var checkContentLength = function(httpRespStream) {
      var expectedLen = httpRespStream.headers['content-length'] ?
        parseInt(httpRespStream.headers['content-length'], 10) :
        undefined;
      var receivedLen = 0;
      var shouldCheckContentLength = Boolean(expectedLen && expectedLen >= 0);
      var contentLengthChecker = new AWS.util.stream.Transform();
      contentLengthChecker.headers = httpRespStream.headers;
      contentLengthChecker.statusCode = httpRespStream.statusCode;
      contentLengthChecker.statusMessage = httpRespStream.statusMessage;
      contentLengthChecker._transform = function(chunk, encoding, callback) {
      ...
      contentLengthChecker.on('end', function() {
        if (shouldCheckContentLength && receivedLen !== expectedLen) {
          contentLengthChecker.emit('error', AWS.util.error(
      ...
      return contentLengthChecker;
    }

    var stream = http.request(options, function (httpResp) {
      if (stream.didCallback) return;
      var contentLengthChecker = checkContentLength(httpResp);
      var checkedStream = httpResp.pipe(contentLengthChecker);
      callback(checkedStream);
      checkedStream.emit(
        'headers',
        httpResp.statusCode,
        httpResp.headers,
