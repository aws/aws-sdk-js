...
IntegrityCheckerStream.prototype._flush = function(callback) {
	if (this.providedChecksum.length !== CHECKSUM_BYTE_LEN) {
		this.emit(
			'error',
			AWS.util.error(
	...
	} else if (this.hash.digest('base64') !== this.providedChecksum.toString('base64')) {
...
IntegrityCheckerStream.prototype._transform = function(chunk, encoding, callback) {
	if (chunk.length > 0) {
		if (this.receivedLen >= this.expectedDataLength) {
			...
			this.providedChecksum.fill(newChecksum, this.providedChecksumLen, this.providedChecksumLen += newChecksum.length);
