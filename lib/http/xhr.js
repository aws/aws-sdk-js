...
AWS.XHRClient = AWS.util.inherit({
  handleRequest: function handleRequest(httpRequest, httpOptions, callback, errCallback) {
    ...
    // console.log('aws fetch ready:',href,httpRequest)
    fetch(request).then(function (response) {
      if (!response.ok) {
        response.text().then(res => {
          console.log("aws res not ok:", res)
        })
      ...
      response.text().then(function (res) {
        //  console.log("aws fetch read success",res);
        self.finishRequest(res, emitter);
      }).catch(function (err) {
        console.log("aws fetch read err", err);
      });
    }).catch(function (err) {
      // console.log(err)
      errCallback(AWS.util.error(new Error('Network Failure:' + JSON.stringify(err)), {
