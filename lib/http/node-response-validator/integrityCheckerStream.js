...
IntegrityCheckerStream.prototype._flush = function(callback) {
	if (this.providedChecksum.length !== 16) {
		this.emit(
			'error',
			AWS.util.error(
...
IntegrityCheckerStream.prototype._transform = function(chunk, encoding, callback) {
	if (chunk.length > 0) {
		if (this.receivedLen >= this.expectedDataLength) {
			this.providedChecksum = this.providedChecksum.fill(
			...
			this.providedChecksum = this.providedChecksum.fill(newChecksum, this.providedChecksumLen, this.providedChecksumLen += newChecksum.length);
