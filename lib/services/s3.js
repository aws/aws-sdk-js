...
AWS.S3 = AWS.Service.defineService('s3', ['2006-03-01'], {
   ...
   * @example Pre-signing a putObject operation with a specific payload (asynchronously)
   *   var params = {Bucket: 'bucket', Key: 'key', Body: 'body'};
  ...
  getSignedUrl: function getSignedUrl(operation, params, callback) {
    ...
    function signedUrlSigner() {
      ...
      // build URL
      request.httpRequest.endpoint.pathname = request.httpRequest.path;
      var querystring = AWS.util.queryParamsToString(queryParams);
      var qSep = request.httpRequest.endpoint.pathname.match('\\?') ? '&' : '?';
      request.httpRequest.endpoint.pathname += qSep + querystring;
    }
