...
AWS.util.update(AWS.S3.prototype, {
  ...
  populateUriFromAccessPointArn: function populateUriFromAccessPointArn(req) {
    ...
    var region = useArnRegion ? accessPointArn.region : req.service.config.region;

    var hostNameParts = [];
    if (accessPointArn.service === 's3-outposts') {
      var serviceName = 's3-outposts';
      var outpostsSuffix = '.' + accessPointArn.outpostId;
      hostNameParts = [
        accessPointArn.accessPoint + '-' + accessPointArn.accountId + outpostsSuffix,
        serviceName
      ];
    } else if (accessPointArn.service === 's3-object-lambda') {
      ...
      var serviceName = 's3-accesspoint';
      var dualStackSuffix = req.service.config.useDualstack ? '.dualstack' : '';
      hostNameParts = [
        accessPointArn.accessPoint + '-' + accessPointArn.accountId,
        serviceName + dualStackSuffix,
      ];
    }
    hostNameParts.push(region);
    hostNameParts.push(dnsSuffix);

    endpoint.hostname = hostNameParts.join('.');
    endpoint.host = endpoint.hostname;
