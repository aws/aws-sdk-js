...
AWS.util.update(AWS.S3.prototype, {
  ...
  validateService: function validateService() {
    ...
    if (this.config.endpoint) this._explicitEndpoint = true;
  },
  ...
  populateURI: function populateURI(req) {
    ...
    if (b) {
      if (!req.service.pathStyleBucketName(b)) {
        if (!req.service._explicitEndpoint) {
  ...
  pathStyleBucketName: function pathStyleBucketName(bucketName) {
    ...
    // never use path style if endpoint was provided directly
    if (this._explicitEndpoint) return false;
