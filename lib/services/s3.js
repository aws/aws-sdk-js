...
AWS.util.update(AWS.S3.prototype, {
  ...
  writeGetObjectResponse: function writeGetObjectResponse(params, callback) {
    var objectLambdaPrefix = 's3-object-lambda';

    /**
     * must do specific copy on api AWS.util.copy is not deep copy and
     * we do not want to corrupt the client api for additional non-objectlambda requests.
     */
    var objectLambdaClient = AWS.util.copy(this);
    objectLambdaClient.api = AWS.util.copy(this.api);
    objectLambdaClient.api.endpointPrefix = objectLambdaPrefix;
    var request = objectLambdaClient.makeRequest('writeGetObjectResponse', AWS.util.copy(params), callback);

    var hostname = objectLambdaClient.endpoint.host;
    if (hostname.includes(objectLambdaClient.config.region)) {
      // hostname specifies a region already
      hostname = hostname.replace('s3.', objectLambdaPrefix + '.');
    } else {
      // Hostname doesn't have a region.
      // Object Lambda requires an explicit region.
      hostname = hostname.replace('s3.', objectLambdaPrefix + '.' + objectLambdaClient.config.region + '.');
    }

    request.httpRequest.endpoint = new AWS.Endpoint(hostname, objectLambdaClient.config);
    return request;
  },
