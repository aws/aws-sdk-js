    ...
    if (params.AccountId) {
      ...
      if (accountId.length < 1 || accountId.length > 63) {
        throw AWS.util.error(
          new Error(),
          {code: 'ValidationError', message: 'Account Id length should bigger than 0 and smaller than 64'}
        );
      }
      //validate pattern
      var hostPattern = /^[a-zA-Z0-9]([a-zA-Z0-9\-])*[a-zA-Z0-9]$/;
      if (!hostPattern.test(accountId)
      ) {
        throw AWS.util.error(new Error(),
          {code: 'ValidationError', message: 'AccountId should be hostname compatible'});
      }
    }
  }
});
