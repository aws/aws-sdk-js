...
AWS.Request = inherit({
  ...
  createReadStream: function createReadStream() {
    // Use ponyfill: https://github.com/MattiasBuelens/web-streams-polyfill
    ...
    if (AWS.HttpClient.streamsApiVersion === 3) {
      ...
      // which enqueues chunks unchanged from the writable side to the readable side.
    ...
    this.on('error', function(err) {
      if (AWS.HttpClient.streamsApiVersion === 3) {
        if (stream.writable && stream.writable.abort) stream.writable.abort(String(err));
      } else {
        ...
        if (AWS.HttpClient.streamsApiVersion === 3) {
          ...
          if (shouldCheckContentLength && typeof TransformStream !== 'undefined') {
            ...
            readableStream = readableStream.pipeThrough ?
              readableStream.pipeThrough(lengthAccumulator) :
              pipeThrough(readableStream, lengthAccumulator);
          }

          if (stream.writable) {
            var promise = readableStream.pipeTo ?
              readableStream.pipeTo(stream.writable) :
              pipeTo(readableStream, stream.writable);
    ...
    return AWS.HttpClient.streamsApiVersion === 3 ? (stream.readable) : stream;
  },
