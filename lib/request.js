...
function isTerminalState(machine) {
  return hardErrorStates[machine._asm.currentState] === 1;
}
    ...
    try {
      ...
      self.emit(self._asm.currentState, function() {
        ...
        if (nextError && isTerminalState(self)) {
          if (origError !== nextError) self._hardError = true;

          // error did not change to complete, no need to pass this as an
          // uncaughtException
          if (!self._hardError) nextError = null;
        }
