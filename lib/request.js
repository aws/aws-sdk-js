...
AWS.Request = inherit({
  ...
  createReadStream: function createReadStream() {
    ...
    var httpData = function(data) { stream.emit('data', data); };
    var httpDone = function() { stream.emit('end'); stream.readable = false; };

    this.on('httpHeaders', function(statusCode) {
      if (statusCode < 300) {
        // Remove httpData listener, add streaming listeners
        req.removeListener('httpData', AWS.EventListeners.Core.HTTP_DATA);
        req.on('httpData', httpData);
        req.on('httpDone', httpDone);

        // Reset listeners if we retry
        req.on('retry', function() {
          req.on('httpData', AWS.EventListeners.Core.HTTP_DATA);
          req.removeListener('httpData', httpData);
          req.removeListener('httpDone', httpDone);
        });
      }
    });
