...
module.exports = AWS.util.inherit({
  constructor: function SharedIniFile() {
    ...
    //ignore exceptions if defaults cannot be loaded
    try {
      var defaults = {};
      var isConfig = true;
      var configFilePath = this.getDefaultFilePath(isConfig);
      defaults[configFilePath] = this.parseFile(configFilePath, isConfig);
      isConfig = false
      var credentialFilePath = this.getDefaultFilePath(isConfig);
      defaults[credentialFilePath] = this.parseFile(credentialFilePath, isConfig);
      this.resolvedProfiles = defaults;
    } catch (e) {}
  },
