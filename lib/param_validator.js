...
AWS.ParamValidator = AWS.util.inherit({
  validate: function validate(rules, params, api, context) {
  ...
  validateStructure: function validateStructure(rules, params, context) {
    ...
    for (paramName in params) {
      ...
      if (paramRules !== undefined) {
        ...
        params[paramName] = this.validateMember(paramRules, paramValue, memberContext);
      } else {
  ...
  validateList: function validateList(rules, params, context) {
    ...
    for (var i = 0; i < params.length; i++) {
      params[i] = this.validateMember(rules, params[i], context + '[' + i + ']');
    }
  ...
  validateMap: function validateMap(rules, params, context) {
    ...
    for (var param in params) {
      ...
      params[param] = this.validateMember(rules, params[param],
                                         context + '[\'' +  param + '\']');
    }
  ...
  validateScalar: function validateScalar(rules, value, context) {
    ...
    switch (rules.type) {
      ...
      case 'timestamp': {
        ...
        value = (value instanceof Date) ? AWS.util.date[this.api.timestampFormat](value)
                                        : value;

        if (this.api.timestampFormat === 'unixTimestamp') 
            return this.validateType(context, value, ['number']);
        else 
            return this.validateType(context, value 
  ...
  validateType: function validateType(context, value, acceptedTypes, type) {
    ...
    for (var i = 0; i < acceptedTypes.length; i++) {
      if (typeof acceptedTypes[i] === 'string') {
        ...
        if ((value || '').match(acceptedTypes[i]))
          return value;
      } else {
        if (value instanceof acceptedTypes[i])
          return value;
        if (AWS.util.isType(value, acceptedTypes[i].name))
          return value;
