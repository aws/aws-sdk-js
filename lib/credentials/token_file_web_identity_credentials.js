...
AWS.TokenFileWebIdentityCredentials = AWS.util.inherit(AWS.Credentials, {
   ...
   * @api private
  */
  assumeRoleChaining: function assumeRoleChaining(paramsArray) {
    var self = this;
    if (paramsArray.length === 0) {
      self.service.credentialsFrom(self.data, self);
    } else {
      var params = paramsArray.shift();
      self.service.credentialsFrom(self.data, self.service.config.credentials);
        ...
        function (err, data) {
          ...
          if (!err) {
            ...
            if (paramsArray.length === 0) {
              ...
              self.assumeRoleChaining(paramsArray);
            }
          }
          callback(err);
        }
