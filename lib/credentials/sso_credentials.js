...
AWS.SingleSignOnCredentials = AWS.util.inherit(AWS.Credentials, {
  ...
  constructor: function SingleSignOnCredentials(options) {
    ...
    //this.expired = true; // TODO: test these today with expired credentials, and see if putting this as true here does anything (judging from other credentials, it doesn't)
  ...
  load: function load(callback) {
    ...
    try {
      ...
      if (!self.service || self.service.config.region !== profile.sso_region) {
        self.service = new SSO({ region: profile.sso_region });
      }
      var request = {
