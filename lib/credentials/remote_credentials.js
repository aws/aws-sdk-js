...
AWS.RemoteCredentials = AWS.util.inherit(AWS.Credentials, {
   ...
   * @api private
   */
  getECSAuthToken: async function getECSAuthToken() {
    if (process && process.env && (process.env[ENV_FULL_URI] || process.env[ENV_AUTH_TOKEN_FILE])) {

      if (process.env[ENV_AUTH_TOKEN_FILE])
        try {
          const data = await fs.readFileSync(process.env[ENV_AUTH_TOKEN_FILE]).toString();
            return data;
        } catch (error) {
   ...
   * @api private
   */
  request: async function request(url, callback) {
    var httpRequest = new AWS.HttpRequest(url);
    httpRequest.method = 'GET';
    httpRequest.headers.Accept = 'application/json';
    var token = await this.getECSAuthToken();
    if (token) {
