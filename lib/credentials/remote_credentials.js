...
AWS.RemoteCredentials = AWS.util.inherit(AWS.Credentials, {
   ...
   * @api private
   */
  providerName: 'RemoteCredentials',

  /**
   * @api private
   */
  envRelativeUri: 'AWS_REMOTE_CREDENTIALS_RELATIVE_URI',

  /**
   * @api private
   */
  envFullUri: 'AWS_REMOTE_CREDENTIALS_FULL_URI',

  /**
   * @api private
   */
  envAuthToken: 'AWS_REMOTE_AUTHORIZATION_TOKEN',
   ...
   * @api private
   */
  isConfiguredForRemoteCredentials: function isConfiguredForRemoteCredentials() {
    return Boolean(
        process &&
        process.env &&
        (process.env[this.envRelativeUri] || process.env[this.envFullUri])
    );
  },
   ...
   * @api private
   */
  getRemoteFullUri: function getRemoteFullUri() {
    if (process && process.env) {
      var relative = process.env[this.envRelativeUri],
          full = process.env[this.envFullUri];
      if (relative) {
        ...
        if (FULL_URI_ALLOWED_PROTOCOLS.indexOf(parsed.protocol) < 0) {
          throw AWS.util.error(
            new Error('Unsupported protocol:  AWS.' + this.providerName + ' supports '
              + FULL_URI_ALLOWED_PROTOCOLS.join(',') + ' only; '
              + parsed.protocol + ' requested.'),
            { code: this.providerName + 'ProviderFailure' }
          );
        }
        ...
        if (FULL_URI_UNRESTRICTED_PROTOCOLS.indexOf(parsed.protocol) < 0 &&
          ...
          throw AWS.util.error(
            new Error('Unsupported hostname: AWS.' + this.providerName + ' only supports '
   ...
   * @api private
   */
  getRemoteAuthToken: function getRemoteAuthToken() {
    if (process && process.env && process.env[this.envFullUri]) {
      return process.env[this.envAuthToken];
    }
  },
  ...
  request: function request(url, callback) {
    ...
    var token = this.getRemoteAuthToken();
    if (token) {
  ...
  refresh: function refresh(callback) {
    ...
    var fullUri = this.getRemoteFullUri();
    if (fullUri === undefined) {
      callbacks(AWS.util.error(
        new Error('Variable ' + this.envRelativeUri + ' or ' + this.envFullUri +
          ' must be set to use AWS.' + this.providerName + '.'),
        { code: this.providerName + 'ProviderFailure' }
      ));
      return;
    }
...
AWS.ECSCredentials = AWS.util.inherit(AWS.RemoteCredentials, {
  constructor: function RemoteCredentials(options) {
    AWS.util.update(this, options);
    AWS.util.update(this, {
      providerName: 'ECSCredentials',
