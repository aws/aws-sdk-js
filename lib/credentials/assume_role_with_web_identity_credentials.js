...
AWS.AssumeRoleWithWebIdentityCredentials = AWS.util.inherit(AWS.Credentials, {
   ...
   * @api private
   */
  getRandomSessionName: function getRandomSessionName() {
  ...
  load: function load(callback) {
    ...
    try {
      ...
      if (params) {
        ...
        var webIdentityCredentials = new AWS.WebIdentityCredentials({
          WebIdentityToken: oidcToken,
          RoleArn: params.roleArn,
          RoleSessionName: params.roleSessionName || this.getRandomSessionName()
        });
        webIdentityCredentials.refresh(function(webErr) {
          if (!webErr) {
            self.sts.credentialsFrom(webIdentityCredentials.data, self);
          }
          callback(webErr);
        });
      }
