...
AWS.AssumeRoleWithWebIdentityCredentials = AWS.util.inherit(AWS.Credentials, {
   ...
   *  AWS.config.credentials = new AWS.AssumeRoleWithWebIdentityCredentials(
   ...
   * @see AWS.Config
   */
  constructor: function AssumeRoleWithWebIdentityCredentials(clientConfig) {
  ...
  getParamsFromSharedConfig: function getParamsFromSharedConfig() {
    try {
      ...
      if (process.env[AWS.util.configOptInEnv]) {
        profilesFromConfig = AWS.util.iniLoader.loadFrom({
          isConfig: true,
          filename: process.env[AWS.util.sharedConfigFileEnv]
        });
      }
      var profilesFromCreds = AWS.util.iniLoader.loadFrom({
        filename: (process.env[AWS.util.configOptInEnv] && process.env[AWS.util.sharedCredentialsFileEnv])
      });
  ...
  load: function load(callback) {
    ...
    try {
      ...
      if (params) {
        ...
        if (self.webIdentityCredentialsProvider) {
          self.webIdentityCredentialsProvider.service.config.params.WebIdentityToken = oidcToken;
        } else {
          self.webIdentityCredentialsProvider = new AWS.WebIdentityCredentials(
            {
        ...
        self.webIdentityCredentialsProvider.refresh(function(webErr) {
          if (!webErr) {
            self.webIdentityCredentialsProvider.service.credentialsFrom(
              webIdentityCredentials.data,
              self
            );
          }
          callback(webErr);
