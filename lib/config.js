...
AWS.loadConfig = function loadConfig(path) {
  var config = JSON.parse(AWS.util.readFileSync(path, 'utf-8'));
  var fileCreds = new AWS.FileSystemCredentials(path, config.credentials);
  config.credentials = new AWS.DefaultCredentials([fileCreds]);
