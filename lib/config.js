...
AWS.FileSystemConfig = inherit(AWS.Config, {
  constructor: function FileSystemConfig(filename) {
    var data = JSON.parse(this.fs().readFileSync(filename, 'utf-8'));
    if (data.credentials === undefined) data.credentials = null;
    var credentials = new AWS.FileSystemCredentials(filename, data.credentials);
...
AWS.DefaultCredentials = inherit(AWS.Credentials, {
  constructor: function DefaultCredentials() {
    var creds;

    creds = new AWS.EnvironmentCredentials('AWS');
    if (creds.accessKeyId) return creds;

    creds = new AWS.EnvironmentCredentials('AMAZON');
    if (creds.accessKeyId) return creds;
