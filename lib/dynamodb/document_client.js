 ...
 * instead of `AttributeValue`s to simplify the development experience with
 * AWS.DynamoDB. JavaScript objects passed in as parameters are marshalled
 * into `AttributeValue` shapes required by Amazon DynamoDB. Responses from
 * DynamoDB are unmarshalled into plain JavaScript objects by the
 * `DocumentClient`. The `DocumentClient`, does not accept `AttributeValue`s
 * in favor of native JavaScript types.
 *
