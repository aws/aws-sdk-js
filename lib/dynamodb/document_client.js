...
AWS.DynamoDB.DocumentClient = AWS.util.inherit({
  ...
  batchGet: function(params, callback) {
    ...
    self.setupResponse(request, params);
  ...
  setupRequest: function setupRequest(request) {
    ...
    request._events.validate.unshift(function(req) {
      var paramsCopy = AWS.util.copy(req.params);
      req.params = translator.translateInput(paramsCopy, inputShape);
    });
  },
   ...
   * @api private
   */
  setupResponse: function setupResponse(request, origParams) {
    var self = this;
    var translator = self.getTranslator();
    ...
    response.nextPage = function(cb) {
      ...
      if (!response.hasNextPage()) {
        if (cb) cb(response.error, null);
        else if (response.error) throw response.error;
        return null;
      }

      var params = AWS.util.copy(origParams);
      if (!response.nextPageTokens) {
        return cb ? cb(null, null) : null;
      } else {
        var inputTokens = config.inputToken;
        if (typeof inputTokens === 'string') inputTokens = [inputTokens];
        for (var i = 0; i < inputTokens.length; i++) {
          params[inputTokens[i]] = response.nextPageTokens[i];
        }
