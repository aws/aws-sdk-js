...
function generateRegionPrefix(region, options) {
  ...
  if (useFipsEndpoint || isFipsRegion(region)) {
    ...
    if (isFipsUsGovRegion(region) || (useFipsEndpoint && isUsGovRegion(region)))
      return 'fips-us-gov-*';
    if (useFipsEndpoint && isUsIsoRegion(region))
      return 'fips-us-iso-*';
    if (useFipsEndpoint && isUsIsobRegion(region))
      return 'fips-us-isob-*';
    return 'fips-*';
  }
...
function isUsIsoRegion(region) {
  return region && region.startsWith('us-iso-');
}

function isUsIsobRegion(region) {
  return region && region.startsWith('us-isob-');
}
