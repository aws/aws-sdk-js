...
AWS.Endpoint = inherit({
  ...
  constructor: function Endpoint(config, serviceName) {
    ...
    this.protocol = config.sslEnabled ? 'https' : 'http';
    this.hostname = serviceName + '.' + config.region + '.amazonaws.com';
    this.port = config.sslEnabled ? 443 : 80;
  }
...
AWS.RequestHandler = inherit({
  ...
  redirect: function redirect(httpRequest, httpResponse) {
    /*jshint sub:true */
    var url = AWS.util.urlParse(httpResponse.headers['location']);

    httpRequest.endpoint.protocol = url.protocol.replace(/:$/, '');
    httpRequest.endpoint.hostname = url.hostname;
    httpRequest.endpoint.path = url.path;
    if (url.port)
      httpRequest.endpoint.port = url.port;
