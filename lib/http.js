...
AWS.RequestHandler = inherit({
  ...
  sendRequest: function sendRequest(httpRequest) {
    ...
    AWS.HttpClient.getInstance().handleRequest(httpRequest, {
      ...
      onData: function onData(data) {
        ...
        requestHandler.handleHttpData(httpResponse);
      },
  ...
  redirect: function redirect(httpRequest, httpResponse) {
    var redirect = AWS.util.urlParse(httpResponse.headers['location']);

    httpRequest.endpoint.scheme = redirect.protocol.replace(/:$/, '');
    httpRequest.endpoint.host = redirect.hostname;
    httpRequest.endpoint.uri = redirect.path;
    if (redirect.port)
      httpRequest.endpoint.port = redirect.port;
