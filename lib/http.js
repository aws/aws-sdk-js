...
AWS.Endpoint = inherit({
  ...
  constructor: function Endpoint(endpoint, config) {
    // When the endpoint does not specify the scheme we will default
    // the configuration option passed, or to AWS.config.sslEnabled
    // to choose between http and https.
    if (!endpoint.match(/^http/)) {
      if (!config) config = {};
      var useSSL = (config.sslEnabled !== undefined) ?
