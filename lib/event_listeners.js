...
AWS.EventListeners = {
  ...
  Http: new AWS.EventEmitter().addNamedListeners(function(add) {
    ...
    add('RETRY_DELAY_SEND', 'retry', function RETRY_DELAY_SEND(resp, req) {
      var delay = 0;
      if (resp.error) {
      ...
      setTimeout(function() { req.emitEvents(resp, 'send'); }, delay);
    });
