...
AWS.EventListeners = {
  ...
  Logger: new AWS.SequentialExecutor().addNamedListeners(function(add) {
    add('LOG_REQUEST', 'complete', function LOG_REQUEST(resp) {
      var logger = this.service.config.logger;
      if (!logger) return;

      var time = AWS.util.date.getDate().getTime();
      var delta = (time - this.startTime.getTime()) / 1000;
      var ansi = logger.isTTY ? true : false;
      var status = resp.httpResponse.statusCode;
      var params = require('util').inspect(this.params, true, true);

      var message = '';
      if (ansi) message += '\x1B[33m';
      message += '[AWS ' + this.service.serviceIdentifier + ' ' + status;
      message += ' ' + delta.toString() + 's ' + resp.retryCount + ' retries]';
      if (ansi) message += '\x1B[0;1m';
      message += ' ' + this.operation + '(' + params + ')';
