...
AWS.EventListeners = {
  Core: new SequentialExecutor().addNamedListeners(function(add, addAsync) {
    ...
    addAsync('CLOCK_SKEWED', 'retry', function CLOCK_SKEWED(resp, done) {
      if (!resp.error) return;
      if (this.service.clockSkewError(resp.error)) {
        var dateHeader = resp.httpResponse.headers.date ||
          resp.httpResponse.headers.Date;
        var serverTime = Date.parse(dateHeader);
        AWS.util.adjustClock(serverTime, done);
      }
    });
