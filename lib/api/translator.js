...
function Translator(api, options) {
  ...
  Object.keys(api.operations).forEach(function (key) {
    var methodName = key[0].toLowerCase() + key.substr(1);
    methodName = methodName.replace(/\d{4}_\d{2}_\d{2}$/, '');
    var operation = new Operation(api.operations[key], options);
...
var string = JSON.stringify(new Translator(JSON.parse(file), {documentation: false}), null, "  ");
string = string.replace(/"(\w+)": /g, function (_, m) { return m === 'default' ? _ : m + ": "; }).replace(/"/g, '\'');
console.log(string);
