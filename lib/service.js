...
AWS.Service = inherit({
  ...
  /**
   * Gets the signer for a given request
   * @param request [AWS.Request]
   * @api private
   */
  getSigner: function getSigner(request) {
    var sigVersion = this.api.signatureVersion;
    var SignerClass = AWS.Signers.RequestSigner.getVersion(sigVersion);
    return new SignerClass(request.httpRequest,
      this.api.signingName || this.api.endpointPrefix);
  },
