...
AWS.Service = inherit({
  ...
  constructor: function Service(config) {
    ...
    if (config.endpoint) {
      this.endpoint = new AWS.Endpoint(config.endpoint, config);
    } else {
      var endpoint = this.serviceName + '.' + config.region + '.amazonaws.com';
      this.endpoint = new AWS.Endpoint(endpoint, config);
    }
  },
