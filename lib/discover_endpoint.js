...
function INVALIDATE_CACHED_ENDPOINT(response) {
  ...
  if (error &&
    ...
    var cacheKey = getCacheKey(request, inputShape);
...
function hasCustomEndpoint(client) {
  ...
  if (client._originalConfig.endpoint && client.config.endpointDiscoveryEnabled === true) {
  ...
  return AWS.config.endpoint || svcConfig.endpoint || client._originalConfig.endpoint;
}

function discoverEndpoint (request, done) {
  var service = request.service || {};
  if (hasCustomEndpoint(service)) return done();
  ...
  switch (isEndpointDiscoveryRequired) {
    case 'OPTIONAL':
      if (service.config.endpointDiscoveryEnabled === true) {
        ...
        request.addNamedListener('INVALIDATE_CACHED_ENDPOINT', 'validateResponse', INVALIDATE_CACHED_ENDPOINT);
      }
      done();
      break;
    case 'REQUIRED':
