...
function isEndpointDiscoveryApplicable(request) {
  ...
  var configFile = iniLoader.loadFrom({
    isConfig: true,
  ...
  var isEnabled = process.env.AWS_ENABLE_ENDPOINT_DISCOVERY || sharedFileConfig.endpoint_discovery_enabled;
  var falsyNotations = ['false', '0', undefined];
  if (falsyNotations.indexOf(isEnabled) >= 0) return false;
  return true;
}
...
function discoverEndpoint (request, done) {
  ...
  switch (isEndpointDiscoveryRequired) {
    case 'OPTIONAL':
      if (isEndpointDiscoveryApplicable(request)) {
