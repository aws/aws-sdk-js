...
var util = {
   ...
   * @api private
   */
  adjustClock: function adjustClock(time, done) {
    time = time || AWS.config.cachedServerTime;
    if (!time) {
      var service = new AWS.CognitoIdentity({paramValidation: false});
      service.getId(function() {
        var serverTime = Date.parse(this.httpResponse.headers.date);
        util.setClockOffset(serverTime);
        done();
      });
    } else {
      util.setClockOffset(time);
      done();
    }
  },
   ...
   * @api private
   */
  setClockOffset: function(time) {
    AWS.config.systemClockOffset = time - new Date().getTime();
    if (AWS.config.cachedServerTime)
      AWS.config.cachedServerTime = undefined;
  }
