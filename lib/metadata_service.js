...
AWS.MetadataService = inherit({
   ...
   */
  host: getMetadataServiceEndpoint().substr(7),
  ...
  request: function request(path, options, callback) {
    ...
    var endpoint = 'http://' + this.host;
    // Verify that host is a valid URL
    var parsedUrl = AWS.util.urlParse(endpoint);
    if (parsedUrl.hostname === null) {
      throw new TypeError('Invalid URL: ' + endpoint);
    }

    var httpRequest = new AWS.HttpRequest(endpoint + path);
