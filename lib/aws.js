...
AWS.DynamoDB.prototype = {
  ...
  listTables: function listTables() {
    ...
    this.config.handler.handleRequest(httpRequest, {
      headers: function headers(statusCode, headers) {
      ...
      data: function data(data) {
        ...
        awsRequest.notifyDone(httpResponse);
      },
...
AWS.HttpHandler.prototype = {
  handleRequest: function handleRequest(request, callbacks) {
    ...
    var req = https.request(options, function callback(res) {
      callbacks.headers(res.statusCode, res.headers);
      res.on('data', callbacks.data);
      res.on('end', callbacks.end);
    });

    req.write(request.body);
    req.end();
    req.on('error', function(e) {
      callbacks.error({ code: 'NetworkingError', message: e.toString() });
    });
...
AWS.AWSRequest.prototype = {
  ...
  notifyDone: function notifyDone(data) {
    this.data = data;
    this.state = 'done';
    this.notify(this.doneCallbacks, [data])
    this.notify(this.alwaysCallbacks, [null, data]);
  },
