...
function ManagedUpload(svc, options) {
  ...
  var callback, body;
  var maxQueue = 4;
  var partSize = ManagedUpload.minPartSize;
  var leavePartsOnError = false;
  ...
  function cleanup(err) {
    ...
    if (svc.config.params.UploadId && !leavePartsOnError) {
      svc.abortMultipartUpload().send();
    }
  ...
  function nextChunk(chunk) {
    ...
    if (!svc.config.params.UploadId) {
      ...
      if (!multipartReq) { // create multipart
        ...
        queueChunks(chunk, partNumber);
        multipartReq.on('error', cleanup);
        multipartReq.send();
      } else {
        queueChunks(chunk, partNumber);
      }
  ...
  var queueFillFns = {
    ...
    buffer: function() {
      ...
      while (activeParts < maxQueue && partPos < bodyLen) {
        ...
        if (AWS.util.string.byteLength(buf) < partSize || partPos === bodyLen) {
          isDoneChunking = true;
  ...
  function configure(opts) {
    ...
    if (opts.leavePartsOnError) leavePartsOnError = true;

    if (partSize < ManagedUpload.minPartSize) {
      throw new Error('partSize must be greater than ' +
                      ManagedUpload.minPartSize);
    }
  }
  ...
  function send(params, cb) {
    ...
    if (totalBytes) {
      var newPartSize = Math.ceil(totalBytes / ManagedUpload.maxTotalParts);
...
ManagedUpload.minPartSize = 1024 * 1024 * 5;
ManagedUpload.maxTotalParts = 10000;
