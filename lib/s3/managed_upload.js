...
AWS.S3.ManagedUpload = AWS.util.inherit({
  constructor: function ManagedUpload(svc, options) {
    var self = this;
  ...
  send: function(params, cb) {
    ...
    self.bindServiceObject(params);
    self.validateBody();
    self.adjustTotalBytes();
  ...
  fillBuffer: function fillBuffer() {
    ...
    while (self.activeParts < self.maxQueue && self.partPos < bodyLen) {
  ...
  fillStream: function fillStream() {
    var self = this;
    if (self.activeParts >= self.maxQueue) return;
