...
AWS.ServiceInterface.Rest = {
  buildRequest: function buildRequest(resp, req) {
    AWS.ServiceInterface.Rest.populateMethod(resp, req);
    AWS.ServiceInterface.Rest.populateURI(resp, req);
    AWS.ServiceInterface.Rest.populateHeaders(resp, req);
  },

  extractError: function extractError() {
  },

  extractData: function extractData(resp, req) {
    var data = {};
  ...
  populateMethod: function populateMethod(resp, req) {
    resp.httpRequest.method = req.client.api.operations[req.operation].m;
  },

  populateURI: function populateURI(resp, req) {
    var operation = req.client.api.operations[req.operation];
    var uri = operation.u;
  ...
  populateHeaders: function populateHeaders(resp, req) {
    var operation = req.client.api.operations[req.operation];
    var rules = (operation.i || {}).m || {};
      ...
      if (rule.l === 'header' && req.params[name]) {
        if (rule.t === 'm') { // header map
          AWS.util.each(req.params[name], function (key, value) {
            resp.httpRequest.headers[rule.n + key] = value;
          });
        } else {
