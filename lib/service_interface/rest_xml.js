...
AWS.ServiceInterface.RestXml = {
  ...
  extractData: function extractData(resp) {
    ...
    if (operation.output_payload) {
      ...
      if (rules[operation.output_payload] && !rules[operation.output_payload].streaming) {
  ...
  populateBody: function populateBody(req) {
    ...
    var body = null;
    var xmlWrapper = input.name;

    if (!xmlWrapper) {
      AWS.util.each(rules, function(name, rule) {
        if ((rule.type == 'structure' && rule.location == 'body') || !rule.location) {
          xmlWrapper = name;
        }
      });
    }
    ...
    if (xmlWrapper) {
      var xmlRules = rules[xmlWrapper] ? (rules[xmlWrapper].members || {}) : rules;
