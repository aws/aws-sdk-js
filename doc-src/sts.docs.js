 ...
 *           Secret Access Key to sign requests.
 *         * `SessionToken` &mdash; **required** &mdash; (`String`) The
 *           security token that users must pass to the service API to use
 *           the temporary credentials.
 *         * `Expiration` &mdash; **required** &mdash; (`Date`) The date on
 *           which these credentials expire.
 ...
 *         * `AssumedRoleId` &mdash; **required** &mdash; (`String`) A
 *           unique identifier that contains the role ID and the role
 *           session name of the role that is being assumed. The role ID
 *           was generated by AWS when the role was created.
 *         * `Arn` &mdash; **required** &mdash; (`String`) The ARN of the
 *           temporary security credentials that are returned from the
 *           AssumeRole action. For more information about ARNs and how to
 *           use them in policies, see Identifiers for IAM Entities in
 *           Using IAM.
 ...
 *         * `FederatedUserId` &mdash; **required** &mdash; (`String`) The
 *           string identifying the federated user associated with the
 *           credentials, similar to the UserId of an IAM user.
 *         * `Arn` &mdash; **required** &mdash; (`String`) The ARN
 *           specifying the federated user associated with the credentials.
