 ...
 *   @param params [Object]
 *     * `InstanceGroups` &mdash; **required** &mdash; (`Array<map>`)
 ...
 *       DescribeJobFlows.
 *     * `Steps` &mdash; **required** &mdash; (`Array<map>`) A list of
 ...
 *           * `CANCEL_AND_WAIT`
 *           * `CONTINUE`
 *         * `HadoopJarStep` &mdash; **required** &mdash; (`map`)
 *           Specifies the JAR file used for the job flow step.
 *             * `Properties` &mdash; (`Array<map>`) A list of Java
 ...
 *       The `data` object has the following properties:
 *
 *       * `JobFlows` &mdash; (`Array<map>`) A list of job flows matching
 ...
 *           * `AmiVersion` &mdash; (`String`) The version of the AMI used
 *             to initialize Amazon EC2 instances in the job flow. For a
 *             list of AMI versions currently supported by Amazon
 *             ElasticMapReduce, go to AMI Versions Supported in Elastic
 *             MapReduce in the Amazon Elastic MapReduce Developer's Guide.
 *           * `ExecutionStatusDetail` &mdash; (`map`) Describes the
 ...
 *                 * `WAITING`
 *                 * `BOOTSTRAPPING`
 *               * `CreationDateTime` &mdash; (`Date`) The creation date
 *                 and time of the job flow.
 *               * `StartDateTime` &mdash; (`Date`) The start date and time
 *                 of the job flow.
 *               * `ReadyDateTime` &mdash; (`Date`) The date and time when
 *                 the job flow was ready to start running bootstrap
 *                 actions.
 *               * `EndDateTime` &mdash; (`Date`) The completion date and
 *                 time of the job flow.
 *               * `LastStateChangeReason` &mdash; (`String`) Description
 *                 of the job flow last changed state.
 *           * `Instances` &mdash; (`map`) Describes the Amazon EC2
 *             instances of the job flow.
 *               * `MasterInstanceType` &mdash; (`String`) The Amazon EC2
 *                 master node instance type.
 *               * `MasterPublicDnsName` &mdash; (`String`) The DNS name of
 *                 the master node.
 *               * `MasterInstanceId` &mdash; (`String`) The Amazon EC2
 *                 instance identifier of the master node.
 *               * `SlaveInstanceType` &mdash; (`String`) The Amazon EC2
 *                 slave node instance type.
 *               * `InstanceCount` &mdash; (`Integer`) The number of Amazon
 *                 EC2 instances in the cluster. If the value is 1, the
 *                 same instance serves as both the master and slave node.
 *                 If the value is greater than 1, one instance is the
 *                 master node and all others are slave nodes.
 *               * `InstanceGroups` &mdash; (`Array<map>`) Details about
 *                 the job flow's instance groups.
 *                   * `InstanceGroupId` &mdash; (`String`) Unique
 *                     identifier for the instance group.
 *                   * `Name` &mdash; (`String`) Friendly name for the
 *                     instance group.
 *                   * `Market` &mdash; (`String`) Market type of the
 *                     Amazon EC2 instances used to create a cluster node.
 *                     Possible values include:
 *                     * `ON_DEMAND`
 *                     * `SPOT`
 *                   * `InstanceRole` &mdash; (`String`) Instance group
 *                     role in the cluster
 *                     Possible values include:
 *                     * `MASTER`
 *                     * `CORE`
 *                     * `TASK`
 *                   * `BidPrice` &mdash; (`String`) Bid price for EC2
 *                     Instances when launching nodes as Spot Instances,
 *                     expressed in USD.
 *                   * `InstanceType` &mdash; (`String`) Amazon EC2
 *                     Instance type.
 *                   * `InstanceRequestCount` &mdash; (`Integer`) Target
 *                     number of instances to run in the instance group.
 *                   * `InstanceRunningCount` &mdash; (`Integer`) Actual
 *                     count of running instances.
 *                   * `State` &mdash; (`String`) State of instance group.
 *                     The following values are deprecated: STARTING,
 ...
 *                   * `CreationDateTime` &mdash; (`Date`) The date/time
 *                     the instance group was created.
 ...
 *                   * `EndDateTime` &mdash; (`Date`) The date/time the
 *                     instance group was terminated.
 *               * `NormalizedInstanceHours` &mdash; (`Integer`) An
 *                 approximation of the cost of the job flow, represented
 *                 in m1.small/hours. This value is incremented once for
 *                 every hour an m1.small runs. Larger instances are
 *                 weighted more, so an Amazon EC2 instance that is roughly
 *                 four times more expensive would result in the normalized
 *                 instance hours being incremented by four. This result is
 *                 only an approximation and does not reflect the actual
 *                 billing rate.
 *               * `Ec2KeyName` &mdash; (`String`) The name of an Amazon
 *                 EC2 key pair that can be used to ssh to the master node
 *                 of job flow.
 *               * `Ec2SubnetId` &mdash; (`String`) For job flows launched
 *                 within Amazon Virtual Private Cloud, this value
 *                 specifies the identifier of the subnet where the job
 *                 flow was launched.
 *               * `Placement` &mdash; (`map`) Specifies the Amazon EC2
 *                 Availability Zone for the job flow.
 *                   * `AvailabilityZone` &mdash; (`String`) The Amazon EC2
 *                     Availability Zone for the job flow.
 *               * `KeepJobFlowAliveWhenNoSteps` &mdash; (`Boolean`)
 *                 Specifies whether or not the job flow should terminate
 *                 after completing all steps.
 *               * `TerminationProtected` &mdash; (`Boolean`) Specifies
 *                 whether the Amazon EC2 instances in the cluster are
 *                 protected from termination by API calls, user
 *                 intervention, or in the event of a job flow error.
 *               * `HadoopVersion` &mdash; (`String`) Specifies the Hadoop
 *                 version for the job flow.
 *           * `Steps` &mdash; (`Array<map>`) A list of steps run by the
 *             job flow.
 *               * `StepConfig` &mdash; (`map`) The step configuration.
 *                   * `Name` &mdash; (`String`) The name of the job flow
 *                     step.
 *                   * `ActionOnFailure` &mdash; (`String`) Specifies the
 *                     action to take if the job flow step fails.
 *                     Possible values include:
 *                     * `TERMINATE_JOB_FLOW`
 *                     * `CANCEL_AND_WAIT`
 *                     * `CONTINUE`
 *                   * `HadoopJarStep` &mdash; (`map`) Specifies the JAR
 *                     file used for the job flow step.
 *                       * `Properties` &mdash; (`Array<map>`) A list of
 *                         Java properties that are set when the step runs.
 *                         You can use these properties to pass key value
 *                         pairs to your main function.
 *                           * `Key` &mdash; (`String`) The unique
 *                             identifier of a key value pair.
 *                           * `Value` &mdash; (`String`) The value part of
 *                             the identified key.
 *                       * `Jar` &mdash; (`String`) A path to a JAR file
 *                         run during the step.
 *                       * `MainClass` &mdash; (`String`) The name of the
 *                         main class in the specified Java file. If not
 *                         specified, the JAR file should specify a
 *                         Main-Class in its manifest file.
 *                       * `Args` &mdash; (`Array<String>`) A list of
 *                         command line arguments passed to the JAR file's
 *                         main function when executed.
 *               * `ExecutionStatusDetail` &mdash; (`map`) The description
 *                 of the step status.
 *                   * `State` &mdash; (`String`) The state of the job flow
 *                     step.
 *                     Possible values include:
 ...
 *                     * `FAILED`
 *                     * `INTERRUPTED`
 *                   * `CreationDateTime` &mdash; (`Date`) The creation
 *                     date and time of the step.
 *                   * `StartDateTime` &mdash; (`Date`) The start date and
 *                     time of the step.
 *                   * `EndDateTime` &mdash; (`Date`) The completion date
 *                     and time of the step.
 *                   * `LastStateChangeReason` &mdash; (`String`) A
 *                     description of the step's current state.
 *           * `BootstrapActions` &mdash; (`Array<map>`) A list of the
 *             bootstrap actions run by the job flow.
 *               * `BootstrapActionConfig` &mdash; (`map`) A description of
 *                 the bootstrap action.
 *                   * `Name` &mdash; (`String`) The name of the bootstrap
 *                     action.
 *                   * `ScriptBootstrapAction` &mdash; (`map`) The script
 *                     run by the bootstrap action.
 *                       * `Path` &mdash; (`String`) Location of the script
 *                         to run during a bootstrap action. Can be either
 *                         a location in Amazon S3 or on a local file
 *                         system.
 *                       * `Args` &mdash; (`Array<String>`) A list of
 *                         command line arguments to pass to the bootstrap
 *                         action script.
 *           * `SupportedProducts` &mdash; (`Array<String>`) A list of
 *             strings set by third party software when the job flow is
 *             launched. If you are not using third party software to
 *             manage the job flow this value is empty.
 *           * `VisibleToAllUsers` &mdash; (`Boolean`) Specifies whether
 *             the job flow is visible to all IAM users of the AWS account
 *             associated with the job flow. If this value is set to true,
 *             all IAM users of that AWS account can view and (if they have
 *             the proper policy permissions set) manage the job flow. If
 *             it is set to false, only the IAM user that created the job
 *             flow can view and manage it. This value can be changed using
 *             the SetVisibleToAllUsers action.
 *           * `JobFlowRole` &mdash; (`String`) The IAM role that was
 *             specified when the job flow was launched. The EC2 instances
 ...
 *   @param params [Object]
 *     * `InstanceGroups` &mdash; (`Array<map>`) Instance groups to
 *       change.
 ...
 *     * `Instances` &mdash; **required** &mdash; (`map`) A specification
 *       of the number and type of Amazon EC2 instances on which to run
 *       the job flow.
 *         * `MasterInstanceType` &mdash; (`String`) The EC2 instance
 *           type of the master node.
 ...
 *         * `InstanceGroups` &mdash; (`Array<map>`) Configuration for
 *           the job flow's instance groups.
 *             * `Name` &mdash; (`String`) Friendly name given to the
 *               instance group.
 *             * `Market` &mdash; (`String`) Market type of the Amazon
 *               EC2 instances used to create a cluster node.
 *               Possible values include:
 *               * `ON_DEMAND`
 *               * `SPOT`
 ...
 *               * `CORE`
 *               * `TASK`
 *             * `BidPrice` &mdash; (`String`) Bid price for each Amazon
 *               EC2 instance in the instance group when launching nodes
 ...
 *           Amazon EC2 key pair that can be used to ssh to the master
 *           node as the user called "hadoop."
 *         * `Placement` &mdash; (`map`) Specifies the Availability Zone
 *           the job flow will run in.
 *             * `AvailabilityZone` &mdash; **required** &mdash;
 *               (`String`) The Amazon EC2 Availability Zone for the job
 *               flow.
 ...
 *           to lock the job flow to prevent the Amazon EC2 instances
 *           from being terminated by API call, user intervention, or in
 *           the event of a job flow error.
 *         * `HadoopVersion` &mdash; (`String`) Specifies the Hadoop
 *           version for the job flow. Valid inputs are "0.18", "0.20",
 *           or "0.20.205". If you do not set this value, the default of
 *           0.18 is used, unless the AmiVersion parameter is set in the
 ...
 *           Amazon Virtual Private Cloud (Amazon VPC), set this
 *           parameter to the identifier of the Amazon VPC subnet where
 *           you want the job flow to launch. If you do not specify this
 *           value, the job flow is launched in the normal Amazon Web
 *           Services cloud, outside of an Amazon VPC. Amazon VPC
 *           currently does not support cluster compute quadruple extra
 *           large (cc1.4xlarge) instances. Thus you cannot specify the
 *           cc1.4xlarge instance type for nodes of a job flow launched
 *           in a Amazon VPC.
 *     * `Steps` &mdash; (`Array<map>`) A list of steps to be executed by
 *       the job flow.
 *         * `Name` &mdash; **required** &mdash; (`String`) The name of
 *           the job flow step.
 *         * `ActionOnFailure` &mdash; (`String`) Specifies the action to
 ...
 *               arguments passed to the JAR file's main function when
 *               executed.
 *     * `BootstrapActions` &mdash; (`Array<map>`) A list of bootstrap
 *       actions that will be run before Hadoop is started on the cluster
 *       nodes.
 *         * `Name` &mdash; **required** &mdash; (`String`) The name of
 *           the bootstrap action.
 *         * `ScriptBootstrapAction` &mdash; **required** &mdash; (`map`)
