...
(function() {
  var AWS, customPolicy, helpers, keyPairId, privateKey, url;
  ...
  describe('AWS.CloudFront.Signer', function() {
    ...
    it('can generate signed cookies with a canned policy', function() {
      ...
      if (AWS.util.isNode()) {
        ...
        return expect(cookieHash).to.include.keys('CloudFront-Key-Pair-Id');
      }
    });
    return it('can generate signed cookies with a custom policy', function() {
      var cookieHash, opts, signer;
      if (AWS.util.isNode()) {
        signer = new AWS.CloudFront.Signer(keyPairId, privateKey);
        opts = {
